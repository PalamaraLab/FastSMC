  ...
  : sites(_sites), totalSamplesBound(_totalSamplesBound), decodingUsesCSFS(_decodingUsesCSFS)
{
  ...
  for (uint i = 0; i < famAndIndNameList.size(); i++) {
    ...
    // individuals[famAndIndNameList[i]] = Individual(FamIDList[i], IIDList[i], sites);
  }
...
void Data::makeUndistinguished(bool foldToMinorAlleles) {
  ...
  for (uint i = 0; i < derivedAlleleCounts.size(); i++) {
    ...
    if (decodingUsesCSFS && totalSamplesBound > totalSamples) {
      cerr << "ERROR. SNP " << SNP_IDs[i] << " has " << totalSamples << " non-missing individuals, but the CSFS requires " << totalSamplesBound << endl;
      exit(1);
    }
...
void Data::readSamplesList(string hapsFileRoot) {
  ...
  //BufferedReader bufferedReader = Utils.openFile(hapsFileRoot + ".samples");
  ...
  //Utils.print("Read " + famAndIndNameList.size() + " samples.");
...
int Data::countHapLines(string hapsFileRoot) {
  ...
  if (fileExists(hapsFileRoot + ".hap.gz")) {
    ...
    cerr << "ERROR. Could not find hap file in " + hapsFileRoot + ".hap.gz, " + hapsFileRoot + ".hap, " + ".haps.gz, or " + hapsFileRoot + ".haps" << endl;
    exit(1);
  }
  string line;
  int pos = 0;
...
void Data::readHaps(string hapsFileRoot, bool foldToMinorAlleles) {
  ...
  string chr, snpID; int bp; string alleleA, alleleB;
  while (hapsBr >> chr >> snpID >> bp >> alleleA >> alleleB) {
    ...
    // cout << "\t" << pos << "\t" << totalSamplesCount[pos] << "\t" << derivedAlleleCounts[pos] << endl;
  ...
  hapsBr.close();
  cout << "Read data for " << famAndIndNameList.size() * 2 << " haploid samples and " << pos << " markers, " << monomorphic << " of which are monomorphic." << endl;
}
